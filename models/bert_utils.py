...
def answer_from_logits(
    ...
    predicted_answers = []
    sampled_answers = {"id": [], "samples": []}  # may be unused
    for example in tqdm(examples):
        ...
        if len(answers) > 0:
            ...
            if multiple_answers:
                ...
                sampled_answers["id"].append(example_id)
                sampled_answers["samples"].append(
                    [(x["text"], x["logit_score"]) for x in answers]
                )
...
def format_tacoma_qa(dataset):
    ...
    idx = Dataset.from_dict({"id": range(len(dataset))})
    dataset = datasets.concatenate_datasets([dataset, idx], axis=1)
